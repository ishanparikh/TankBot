...
i = 0
searchTheSnitch = False;
message = GameServer.readMessage()
tankX = message.get("X")
tankY = message.get("Y")
tankID = message.get("Id")
tankMunition = message.get("Health")
tankLife = message.get("Ammo")
tanksLastPosition = dict()
healthPickupLastPosition = dict()
ammoPickupLastPosition = dict()

control = int(round(time.time() * 1000))

while True:
    xValue = message.get("X")
    yValue = message.get("Y")
    message = GameServer.readMessage()

    observation = message.get("messageType")
    if observation == ServerMessageTypes.OBJECTUPDATE:
        if message.get("Type") == "Tank":
            if message.get("Id") == tankID:
                # Update the variables of our tank
                tankX = message.get("X")
                tankY = message.get("Y")
                tankLife = message.get("Health")
